...
function Canvas() {
  ...
  function mousemove(d) {
    ...
    // if (best.p) {
    //   best.p.sprite.scale.x = best.p.sprite.scale.y = 2;
    //   oldBest = best.p.sprite;
    // } else {
    //   if (oldBest) oldBest.sprite.scale.x = oldBest.sprite.scale.y = 1;
    // }
  ...
  canvas.resetZoom = function () {
    ...
    y = height / -2 + (extent[1] - extent[0] / 2)
  ...
  function filterVisible() {
    ...
    if (visible.length < 400) {
      data.forEach(function (d) {
        if (d.visible && d.loaded2 && d.active) {
          d.alpha2 = 1
          d.alpha = 0
