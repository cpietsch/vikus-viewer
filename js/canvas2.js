...
function Canvas() {
    ...
    var timelineData;
    var stage, stage1, stage2, stage3, stage4, stage44, stage5;
    ...
    canvas.makeScales = function () {
        ...
        scale22 = imageSize22 / (x.rangeBand() / collumns);
        ...
        stage44.scale.x = 1 / scale22;
        stage44.scale.y = 1 / scale22;
        stage44.y = height;
    ...
    canvas.init = function (_data, _timeline, _config) {
        ...
        imageSize22 = config.loader.textures.inter.size;
    ...
    function stackLayout(data, invert) {
        ...
        years.forEach(function (year) {
            ...
            year.values.forEach(function (d, i) {
                ...
                if (d.sprite3) {
                    d.sprite3.position.x = d.x * scale22 + imageSize22 / 2;
                    d.sprite3.position.y = d.y * scale22 + imageSize22 / 2;
                }
    ...
    function imageAnimation() {
        ...
        data.forEach(function (d, i) {
            ...
            if (d.sprite3) {
                diff = (d.alpha3 - d.sprite3.alpha);
                if (Math.abs(diff) > 0.01) {
                    d.sprite3.alpha += diff * 0.2
                    sleep = false;
                }

                d.sprite3.visible = d.sprite3.alpha > 0.01;
                //else d.sprite2.visible = d.visible;
            }
        });
    ...
    function filterVisible() {
        ...
        if (visible.length < 700 && visible.length > 300) {
            data.forEach(function (d) {
                if (d.visible && d.loaded && d.active) {d.alpha3 = 1; d.alpha = 0; }
                else if (d.visible && !d.loaded2 && d.active) loadImagesCue2.push(d);
                else {d.alpha3 = 0; d.alpha = 1;}
            })
        } else if (visible.length < 300) {
            data.forEach(function (d) {
                if (d.visible && d.loaded && d.active) {d.alpha2 = 1; d.alpha3 = 0; d.alpha = 0; }
                else if (d.visible && !d.loaded && d.active) loadImagesCue.push(d);
                else {d.alpha2 = 0; d.alpha3 = 0; d.alpha = 1;}
            })
        } 
        else {
            data.forEach(function (d) {
    ...
    function loadMiddleImage2(d) {
        ...
        console.log("loadMiddleImage2", d)
        var url = config.loader.textures.inter.url + d.id + '.png'
        var texture = new PIXI.Texture.fromImage(url)
        var sprite = new PIXI.Sprite(texture);
        ...
        sprite.anchor.x = 0.5;
        sprite.anchor.y = 0.5;
        sprite.position.x = d.x * scale22 + imageSize22 / 2;
        sprite.position.y = d.y * scale22 + imageSize22 / 2;
        sprite._data = d;
        stage44.addChild(sprite);
        d.sprite3 = sprite;
        d.alpha3 = d.highlight;
        d.loaded2 = true;
        sleep = false
    }
    ...
    function loadImages() {
        ...
        for (let index = 0; index < 4; index++) {
            if (loadImagesCue2.length) {
                var d = loadImagesCue2.pop();
                // console.log(d.id)
                if (!d.loaded2) {
                    loadMiddleImage2(d);
                }
